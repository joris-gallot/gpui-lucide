...
use gpui::{
    App, AppContext, Application, AssetSource, Bounds, Context, FocusHandle, Focusable, Hsla,
    InteractiveElement, IntoElement, KeyBinding, Render, SharedString, StatefulInteractiveElement,
    Styled, Window, WindowBounds, WindowOptions, actions, div, prelude::*, px, radians, rgb,
    uniform_list,
};
...
actions!(playground, [Quit, ClearSearch]);
...
impl Playground {
    fn new(cx: &mut Context<Self>) -> Self {
        let mut app = Self {
            ...
            filtered_icons: IconName::all().collect(),
            hovered_icon: None,
        };
        app.update_filtered_icons();
        app
    }

    fn update_filtered_icons(&mut self) {
        let query = self.search_query.to_lowercase();
        self.filtered_icons = if query.is_empty() {
    ...
    #[allow(dead_code)]
    fn on_search_change(&mut self, new_value: &str, cx: &mut Context<Self>) {
        self.search_query = new_value.to_string();
        self.update_filtered_icons();
        cx.notify();
    }
                ...
                div()
                    ...
                    .child(
                        div()
                            ...
                            .child(format!("{} icons", IconName::count())),
                    ),
            )
            .child(
                div()
...
impl Playground {
    ...
    fn render_search_input(&mut self, _cx: &mut Context<Self>) -> impl IntoElement {
                    ...
                    move |this, visible_range: std::ops::Range<usize>, _window, cx| {
                        visible_range
                            .map(|row_idx| {
                                ...
                                let end_idx =
                                    (start_idx + items_per_row).min(this.filtered_icons.len());
    ...
    Application::with_platform(gpui_platform::current_platform(false))
        ...
        .run(|cx: &mut App| {
            cx.bind_keys([
                ...
                KeyBinding::new("escape", ClearSearch, Some("Playground")),
            ]);
